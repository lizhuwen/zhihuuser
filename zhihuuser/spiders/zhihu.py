...
class ZhihuSpider(Spider):
    ...
    def start_requests(self):
        ...
        yield Request(self.follows_url.format(user=self.start_user, include=self.followers_query, offset=0, limit=20),callback=self.parse_follows)
    ...
    def parse_followers(self, response):
        ...
        if 'data' in results.keys():
            for result in results.get('data'):
                yield Request(self.user_url.format(user=result.get('url_token'), include=self.followers_query), self.parse_user)
